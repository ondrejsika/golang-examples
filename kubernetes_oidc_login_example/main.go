...
func run(issuerURL, clientID string) error {
	cfg, err := discoverOIDC(issuerURL)
	if err != nil {
...
func main() {
	issuerURL := flag.String("oidc-issuer-url", "", "OIDC issuer URL")
	clientID := flag.String("oidc-client-id", "", "OIDC client ID")
	flag.Parse()

	if *issuerURL == "" || *clientID == "" {
		fmt.Fprintln(os.Stderr, "usage: kubernetes_oidc_login_example --oidc-issuer-url=<url> --oidc-client-id=<id>")
		os.Exit(1)
	}
	...
	*issuerURL = strings.TrimRight(*issuerURL, "/")
	...
	if err := run(*issuerURL, *clientID); err != nil {
